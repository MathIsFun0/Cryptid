...
local curate = { --This needs tweaks to make things not spawn only as polychromes or negatives
	--I know lowering the number would fix tis but i'm sure there is a better solution anyway
    	object_type = "Voucher",
	key = "curate",
   	atlas = "atlasvoucher",
	pos = {x = 2, y = 0},
	...
	loc_txt = {
	    ...
	    "all cards",
            "appear with",
	    "an {C:dark_edition}Editon{}"
		}
    },
    redeem = function(self)
        G.E_MANAGER:add_event(Event({func = function()
            G.GAME.edition_rate = 123456789
            return true end }))
    end
}
