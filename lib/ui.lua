...
SMODS.DrawStep({
	...
	func = function(self)
		...
		if currentBack.effect.center.edeck_type or self.edeck_select then
			...
			local sprite = cry_edeck_atlas_update(not self.edeck_select and currentBack.effect.center or self.config.center)
			...
			if currentBack.effect.center.edeck_type == "sticker" or slsticker then
				...
				if sticker ~= "cry_rigged" then
					if sticker == "cry_absolute" then self.children.back:draw_shader("polychrome", nil, self.ARGS.send_to_shader, _no_tilt) end
				...
				elseif sticker == "cry_rigged" then
					...
					G.shared_stickers["cry_riggeddeck"]:draw_shader("dissolve", nil, nil, _no_tilt, self.children.center)
