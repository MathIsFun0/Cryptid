...
local pointer = {
	...
	init = function(self)
		...
		local aliases = {
			...
			jen = "jen walter",
			jen2 = "Jen Walter Wondergeist",
			jen3 = "Jen Walter Wondergeist (Ascended)",
			survivor = "survivor",
			monk = "monk",
			hunter = "hunter",
			gourmand = "gourmand",
			...
			r_fool = "genius",
			scientist = "scientist",
			r_magician = "scientist",
			lowlaywoman = "low laywoman",
			laywoman = "low laywoman",
			r_priestess = "low laywoman",
			peasant = "peasant",
			r_empress = "peasant",
			servant = "servant",
			r_emperor = "servant",
			adversary = "adversary",
			r_hierophant = "adversary",
			rivals = "rivals",
			r_lovers = "rivals",
			hitchhiker = "hitchhiker",
			r_chariot = "hitchhiker",
			injustice = "c_jen_reverse_justice",
			r_justice = "c_jen_reverse_justice",
			extrovert = "extrovert",
			r_hermit = "extrovert",
			discofpenury = "disc of penury",
			r_wheeloffortune = "disc of penury",
			r_wof = "disc of penury",
			infirmity = "infirmity",
		...
		G.FUNCS.pointer_apply = function()
			local function apply_lower(str)
				...
				-- If the string is exactly "joker", return it as-is
				if str:lower() == "joker" then
					return "joker"
				end
		
				 -- Remove the word "the" (case insensitive) if it's part of a longer string
				 str = str:gsub("the", "") -- Remove "the" in any case

				 -- Remove the word "joker" (case insensitive) if it's part of a longer string
				 str = str:gsub("joker", "") -- Remove "joker" in any case
