			...
			if card.ability.immutable.check < card.ability.extra.trigger then --Hardcoded, dont want misprint to mess with this hehe
				return {
					card_eval_status_text(card, "extra", nil, nil, nil, {
						message = number_format(card.ability.immutable.check) .. "/" .. number_format(card.ability.extra.trigger),
						colour = G.C.DARK_EDITION,
					}),
				}
			end
		end
		if context.joker_main and (to_big(card.ability.extra.mult) > to_big(1)) and to_big(card.ability.immutable.check) > to_big(card.ability.extra.trigger) then
			return {
				message = localize({ type = "variable", key = "a_powmult", vars = { number_format(card.ability.extra.mult) } }),
...
local pickle = {
	...
	calculate = function(self, card, context)
		...
		if context.setting_blind and not context.blueprint then
			card.ability.extra.tags = lenient_bignum(card.ability.extra.tags - card.ability.extra.tags_mod)
...
local compound_interest = {
	...
	calc_dollar_bonus = function(self, card)
		if G.GAME.dollars > to_big(0) then
			local bonus = lenient_bignum(math.max(0, math.floor(0.01 * card.ability.extra.percent * (G.GAME.dollars or 1))))
...
local nice = {
	...
	calculate = function(self, card, context)
		...
		elseif context.cardarea == G.jokers and context.joker_main then
			if to_big(card.ability.immutable.sixcount) > to_big(0) and to_big(card.ability.immutable.ninecount) > to_big(0) then
				return {
					message = localize({ type = "variable", key = "a_chips", vars = { number_format(card.ability.extra.chips) } }),
					chip_mod = lenient_bignum(card.ability.extra.chips),
				}
...
local magnet = {
	...
	calc_dollar_bonus = function(self, card)
		if to_big(#G.jokers.cards) <= to_big(card.ability.extra.slots) then
			return lenient_bignum(card.ability.extra.money * card.ability.extra.Xmoney)
		else
		...
		then
			...
			G.E_MANAGER:add_event(Event({
				func = function()
					...
					card.ability.extra.x_mult = lenient_bignum(card.ability.extra.x_mult + sliced_card.sell_cost * 0.2)
			...
			card_eval_status_text(card, "extra", nil, nil, nil, {
				message = localize({
					type = "variable",
					key = "a_xmult",
					vars = { number_format(lenient_bignum(card.ability.extra.x_mult + 0.2 * sliced_card.sell_cost)) },
				}),
				colour = G.C.RED,
				no_juice = true,
			})
	...
	calculate = function(self, card, context)
		...
		then
			...
			G.E_MANAGER:add_event(Event({
				func = function()
					G.GAME.joker_buffer = 0
					card.ability.extra.chips = lenient_bignum(card.ability.extra.chips + sliced_card.sell_cost * 10)
					card:juice_up(0.8, 0.8)
					sliced_card:start_dissolve({ HEX("57ecab") }, nil, 1.6)
					play_sound("slice1", 0.96 + math.random() * 0.08)
					return true
			...
			card_eval_status_text(card, "extra", nil, nil, nil, {
				message = localize({
					type = "variable",
					key = "a_chips",
					vars = { number_format(lenient_bignum(card.ability.extra.chips + 10 * sliced_card.sell_cost)) },
				}),
				colour = G.C.CHIPS,
				no_juice = true,
			})
	...
	calculate = function(self, card, context)
		...
		then
			...
			G.E_MANAGER:add_event(Event({
				func = function()
					G.GAME.joker_buffer = 0
					card.ability.extra.x_chips = lenient_bignum(card.ability.extra.x_chips + sliced_card.sell_cost * 0.25)
					card:juice_up(0.8, 0.8)
					sliced_card:start_dissolve({ HEX("57ecab") }, nil, 1.6)
					play_sound("slice1", 0.96 + math.random() * 0.08)
					return true
			...
			card_eval_status_text(card, "extra", nil, nil, nil, {
				message = localize({
					type = "variable",
					key = "a_xchips",
					vars = { number_format(lenient_bignum(card.ability.extra.x_chips + 0.25 * sliced_card.sell_cost)) },
				}),
				colour = G.C.CHIPS,
				no_juice = true,
			})
	...
	calculate = function(self, card, context)
		...
		then
			if context.other_card.ability.effect ~= "Base" then
				...
				if to_big(card.ability.immutable.score) >= to_big(card.ability.extra.req) and not card.ability.extra.check then
		...
		if context.joker_main and (to_big(card.ability.extra.x_mult) > to_big(1)) then
			if context.poker_hands ~= nil and next(context.poker_hands[card.ability.type]) then
				return {
					message = localize({ type = "variable", key = "a_xmult", vars = { number_format(card.ability.extra.x_mult) } }),
					colour = G.C.RED,
					Xmult_mod = number_format(card.ability.extra.x_mult),
				}
			end
		end
	end,
...
local giggly = {
	...
	calculate = function(self, card, context)
		if context.joker_main and context.poker_hands and next(context.poker_hands["High Card"]) then
			return {
				message = localize({ type = "variable", key = "a_mult", vars = { number_format(card.ability.extra.t_mult) } }),
				colour = G.C.RED,
...
local dubious = {
	...
	calculate = function(self, card, context)
		if context.joker_main and context.poker_hands and next(context.poker_hands["High Card"]) then
			return {
				message = localize({ type = "variable", key = "a_chips", vars = { number_format(card.ability.extra.t_chips) } }),
				colour = G.C.BLUE,
...
local coin = {
	...
	calculate = function(self, card, context)
		if context.selling_card and context.card.ability.set == "Joker" then
			...
			local option = lenient_bignum(card.ability.extra.money * mod)
			ease_dollars(option)
			card_eval_status_text(
				context.blueprint_card or card,
				"extra",
	...
	calculate = function(self, card, context)
		if context.joker_main and (card.ability.extra.mult > 0) then
			...
			then
				...
				return {
					message = localize({ type = "variable", key = "a_mult_minus", vars = { number_format(card.ability.extra.mult) } }),
	...
	loc_vars = function(self, info_queue, card)
		return {
			vars = {
				...
				number_format(lenient_bignum(card.ability.extra.Xmult_mod * Cryptid.member_count))
			}
		}
	end,
	calculate = function(self, card, context)
		if context.joker_main and to_big(card.ability.extra.Xmult_mod) * to_big(Cryptid.member_count) > to_big(1) then
			return {
				message = localize({
					type = "variable",
					key = "a_xmult",
					vars = { number_format(lenient_bignum(card.ability.extra.Xmult_mod * Cryptid.member_count)) },
				}),
				Xmult_mod = lenient_bignum(card.ability.extra.Xmult_mod * Cryptid.member_count),
			}
		end
	end,
	cry_credits = {
...
local kidnap = {
	...
	loc_vars = function(self, info_queue, center)
		...
		return {
			vars = {
				number_format(center.ability.extra.money),
				number_format(lenient_bignum(center.ability.extra.money * value))
			}
		}
	end,
	atlas = "atlasone",
	calc_dollar_bonus = function(self, card)
		...
		return lenient_bignum(card.ability.extra.money * value)
	end,
	cry_credits = {
		idea = {
			"Jevonn",
...
local pumpkin = {
	...
	calculate = function(self, card, context)
		if context.game_over and to_big(G.GAME.chips / G.GAME.blind.chips) >= to_big(card.ability.extra.scoreReq / 100) then
			G.E_MANAGER:add_event(Event({
				func = function()
	...
	calculate = function(self, card, context)
		if context.end_of_round and not context.blueprint and not context.individual and not context.repetition then
			if G.GAME.blind:get_type() == "Boss" then
				card.ability.extra.disables = lenient_bignum(card.ability.extra.disables - 1)
				card:juice_up()
		...
		if context.before and context.cardarea == G.jokers then
			if not (next(context.poker_hands["Straight"]) or next(context.poker_hands["Straight Flush"])) then
				...
				if #unique_ranks >= 1 then
					card.ability.extra.mult = lenient_bignum(card.ability.extra.mult + (#unique_ranks * card.ability.extra.a_mult))
					return {
						message = localize("k_upgrade_ex"),
						colour = G.C.RED,
						card = card,
